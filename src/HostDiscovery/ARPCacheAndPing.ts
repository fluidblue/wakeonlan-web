...
function getSubnetMask(subnet: number): number {
	...
	console.log(getIPFromNumber(subnetMask));
...
function getFirstAddress(ipNetwork: number, subnet: number): number {
	...
	ipNetwork = getCleanNetworkAddress(ipNetwork, subnetMask);
...
class ARPCacheAndPing implements HostDiscovery {
	discover(
		...
		const ipNetwork: number = getNumberFromIP(ipSubnet.ip);
		console.log("\nipNetwork:")
		console.log(ipNetwork);
		console.log(getIPFromNumber(ipNetwork));

		const ipFirst: number = getFirstAddress(ipNetwork, ipSubnet.prefix);
		console.log("\nipStart:")
		console.log(ipFirst);
		console.log(getIPFromNumber(ipFirst));

		const ipLast: number = getLastAddress(ipNetwork, ipSubnet.prefix);
		console.log("\nipEnd:")
		console.log(ipLast);
		console.log(getIPFromNumber(ipLast));
		...
		for (let ip = ipFirst; ip <= ipLast; ip++) {
			// TODO
			console.log(getIPFromNumber(ip));
